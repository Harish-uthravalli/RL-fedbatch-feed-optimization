...
class EpisodeBasedEvalCallback(BaseCallback):
    def __init__(self, eval_env, experiment_name,  model_dir, eval_interval=20, n_eval_episodes=5, patience=20, verbose=1):
    ...
    def _on_step(self) -> bool:
        if self.locals["dones"][0]:
            ...
            if self.episode_count % self.eval_interval == 0:
                ...
                self.run_rmse_analysis(runs=10)
    ...
    def run_rmse_analysis(self, runs = 10):
        rmses = []
        for i in range(runs):
            ...
            while not done:
                ...
                if done:
                    substrates = info['substrate']
                    cells = info['cells']
                    scrs = substrates/cells
                    scr_mean = sum(scrs)/len(scrs)
                    rmses.append(scr_mean)
        scr_mean = sum(rmses)/len(rmses)

        df = pd.DataFrame([[self.episode_count, scr_mean]], columns=['episode', 'scr_mean'])
    
        save_path = os.path.join('experiments', self.experiment_name, 'rmse_data.csv')

        # Write header only if file doesn't exist
        write_header = not os.path.isfile(save_path)

        # Append data without repeating the header
